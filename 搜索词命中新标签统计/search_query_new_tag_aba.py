...
def analyzer_query_tag_relation(query=None):
    """
    :param query:
    :return:
    """
...
def save_tag(key, com_tagname, first_demands_name_list=[], second_demands_name_list=[],
             first_solutions_name_list=[], second_solutions_name_list=[], first_positions_name_list=[],
             second_positions_name_list=[], tagv3_name_list=[], first_demands_ids_list=[],
        ...
        for item in highlight:
            for key, value in item.items():
                for tag_name in value:
                    ...
                    if ems_tagname == query or (query in ems_tagname) or (query in tag):
                        print("----------------ems_tagname:%s,query:%s,tag:%s" % (ems_tagname, query, tag))
                        [tagv3_name_list, first_solutions_name_list, second_solutions_name_list,
                         ...
                         second_demands_ids_list, first_solutions_ids_list, second_solutions_ids_list] = save_tag(
                            key=key, com_tagname=ems_tagname, first_demands_name_list=first_demands_name_list,
                            ...
                            second_positions_name_list=second_positions_name_list, tagv3_name_list=tagv3_name_list,
                            first_demands_ids_list=first_demands_ids_list,
                            second_demands_ids_list=second_demands_ids_list,
                            first_solutions_ids_list=first_solutions_ids_list,
                            second_solutions_ids_list=second_solutions_ids_list,
                            first_positions_ids_list=first_positions_ids_list,
                            second_positions_ids_list=second_positions_ids_list,
                            tagv3_ids_list=tagv3_ids_list)
                    else:
                        print("------wwwwwwwwwwwwwwww----------ems_tagname:%s,query:%s,tag:%s" % (
                            ems_tagname, query, tag))
                        ...
                        if bol_tag:
                            [tagv3_name_list, first_solutions_name_list, second_solutions_name_list,
                             ...
                             second_positions_name_list,
                             tagv3_ids_list, first_positions_ids_list, second_positions_ids_list,
                             first_demands_ids_list,
                             second_demands_ids_list, first_solutions_ids_list, second_solutions_ids_list] = save_tag(
                                key=key, com_tagname=ems_tagname, first_demands_name_list=first_demands_name_list,
                                second_demands_name_list=second_demands_name_list,
                                ...
                                second_positions_ids_list=second_positions_ids_list,
                                tagv3_ids_list=tagv3_ids_list)
        print("query:%s,******************,highlight:%s" % (
            query, {"tagv3_name_list": tagv3_name_list, "first_solutions_name_list": first_solutions_name_list,
                    "second_solutions_name_list": second_solutions_name_list,
    ...
    try:
        for entity_type in word_attr_info_dict:
            for item in word_attr_info_dict[entity_type]:
                item = item.decode()
                if entity_type == "tag_v3" and item not in tagv3_name_list:
                ...
                if entity_type == "first_position" and item not in first_positions_name_list:
        ...
        return {"tagv3_name_list": tagv3_name_list, "first_demands_name_list": first_demands_name_list,
                "second_demands_name_list": second_demands_name_list,
                "first_solutions_name_list": first_solutions_name_list,
                "second_solutions_name_list": second_solutions_name_list,
                "first_positions_name_list": first_positions_name_list,
...
def query_filter_tractate_v1(query=None):
    try:
        ...
        res = es.indices.analyze(index="gm-dbmw-tractate", body=body)
        ...
        tractate_extra = []
        tractate_dict = {}
        res = es.search(index="gm-dbmw-tractate-read", doc_type="tractate", body=q)
        res_hit = res["hits"]["hits"]
        for item in res_hit:
            if '_source' in item:
                highlight = item.get('highlight')
                if highlight != None:
                    for k, v in highlight.keys():
                        if k not in tractate_dict.keys():
                            tractate_dict[k] = v
                        else:
                            for i in v:
                                if i not in tractate_dict[k]:
                                    tractate_dict[k].append(i)
...
if __name__ == "__main__":
    ...
    for item in all_query_list:
        ...
        print("item:%s,word_attr_info_dict:%s,tagv3_name_list:%s" % (item, word_attr_info_dict, tagv3_name_list))
        ...
        relation_tags = {}
        for tag_key, tag_val in all_tag_info.items():
            for j in tag_val:
                tractate_extra, all_token_dict = query_filter_tractate_v1(query=j)
                target_tag = get_target_tag(tractate_extra, all_token_dict, j)
                print("---:q!:q!-------------")
                print("query:%s,target_tag:%s,all_token_dict:%s,tractate_extra:%s" % (
                    j, target_tag, all_token_dict, tractate_extra))
                for key, value in target_tag.items():
                    if key not in relation_tags.keys():
                        relation_tags[key] = value
                    else:
                        for ids in value:
                            if ids not in relation_tags[key]:
                                relation_tags[key].append(ids)
