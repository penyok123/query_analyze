...
zhengxing_conn = pymysql.connect(
    ...
    database="zhengxing", charset="utf8")
    ...
    if len(split_list) == 2:
        ...
        while end < word_len:
            ...
            if entity_type in ["city", "doctor", "hospital"]:
                bad_word = True
                break
        if len(word) > 30 or int(counts) < 3:
            bad_word = True
...
if __name__ == "__main__":
    ...
    ori_guangyu_query_word_fd = open("/data/log/spider/test_service/4.txt", "r")
    ori_query_word_fd = open("/data/log/word2vec/5.txt", "r")
    offi_query_word_fd = open("./3.txt", "w")
